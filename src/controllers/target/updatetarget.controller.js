...
const updateUserTargetComment = asyncHandler(async (req, res) => {
  const session = await mongoose.startSession();
  session.startTransaction();

  try {
    const { userTargets, monthIndex } = req.body;
    if (!userTargets || !Array.isArray(userTargets) || !monthIndex) {
      await session.abortTransaction();
      session.endSession();
      return res
    ...
    const MonthName = getMonthName(month);
    // const validUserIds = [];
    for (const { userId, newTargetValue, comment } of userTargets) {
      // if (!comment) {
      //   await session.abortTransaction();
      //   session.endSession();
      //   return res
      //     .status(400)
      //     .json(new ApiResponse(400, {}, `Comment key is not provided`));
      // }
      const user = await User.findOne({ _id: userId }).session(session);
      if (!user) {
        await session.abortTransaction();
      ...
      existingTarget.updatedBy = loggedInUser.name;
      if (comment) {
        existingTarget.comment = comment;
      }
...
export { updateUserTarget, updateUserTargetComment };

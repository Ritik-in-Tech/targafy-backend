...
export const getSubGroupData = asyncHandler(async (req, res) => {
  try {
    const { businessId, paramName, monthValue, groupId } = req.params;
    ...
    const totalTargetValue = await GetTargetAssignedUsers(
      paramName,
      monthValue,
      businessId,
      lastDayOfMonth1,
    ...
    const data = {
      ...
      dailyTargetAccumulated: [
        ...
        [lastDayOfMonth.format("YYYY-MM-DD"), totalTargetValue],
      ],
    };

    let percentage;
    percentage = (totalTargetAchieved / totalTargetValue) * 100;
    ...
    return res.status(200).json(
      new ApiResponse(
        ...
        `${groupDetail.groupName} and below group data fetched successfully`
      )
    );
  } catch (error) {
    console.log(error);
